...
TEMPLATES_REPO="${TEMPLATES_REPO:-SamFleming-TylerTech/fork-action-sync-templates}"
...
OPTIONS:
    ...
    --templates-repo <r>   Central templates repo (default: SamFleming-TylerTech/fork-action-sync-templates, or TEMPLATES_REPO env var)
...
copy_and_substitute_workflows() {
    ...
    for src_name in "${!caller_map[@]}"; do
        ...
        success "  Copied and configured: .github/workflows/${dst_name} (caller -> ${TEMPLATES_REPO}@${TEMPLATES_REF})"
    done
}
...
sync_infra_exists() {
    ...
    [[ -f "${CLONE_DIR}/.github/workflows/security-scan.yml" ]]
}
...
print_summary() {
    ...
    echo "    3. Add required secrets (if any) under repository settings."
